...
public class AIPlayer extends Player{
    ...
    public void movePiece(int col) {
        ...
        if(board.isLegalMove(col)){
            board.updateMove(col,CubeColor.GREEN);
            ...
            if(winner!=null){
                ...
                if(!board.existLegalMoves()){
                    board.getBoardUI().notifyWinner(new Winner(CubeColor.EMPTY));
                }
            }
        }
    }
    ...
    public int findColumn(){
        CubeColor[][] pieces = BoardImpl.getPieces();
        for (int col= 0; col < 6; col++) {
            for (int row = 0; row < 5; row++) {
                if (pieces[col][row] == CubeColor.BLUE) {
                    virtualBoard[col][row] = -1;
                } else if (pieces[col][row] == CubeColor.GREEN) {
                    virtualBoard[col][row] = 1;
                } else if (pieces[row][row] == CubeColor.EMPTY)
